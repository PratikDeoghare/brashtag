...
func toHtml(r bt.Node, footnotes map[string]bt.Node) string {
	...
	case bt.Bag:
		...
		if strings.HasPrefix(x.Tag(), "youtube") {
			return fmt.Sprintf(
				`<div class="youtube"> <iframe src="https://youtu.be/%s" /> </div>`,
				getText(x))
		}
